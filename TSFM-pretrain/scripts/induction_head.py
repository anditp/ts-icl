...
def main():
    ...
    d_patch = int(model_cfg.d_patch)

    if args.build_dataset_only:
        build_and_save_dataset_bundle(
        ...
        if args.plot_examples_file is not None:
            bundle = torch.load(args.dataset_file, map_location="cpu")
            plot_dataset_examples(bundle, args.plot_examples_file)
        return

    if args.checkpoint_dir is None:
        raise ValueError("--checkpoint_dir is required when not using --build_dataset_only")
    if not os.path.exists(args.dataset_file):
        raise FileNotFoundError(
            f"Dataset file not found: {args.dataset_file}. Run once with --build_dataset_only."
        )
    ...
    ckpts = sorted(
        glob.glob(os.path.join(args.checkpoint_dir, "step-*.ckpt")),
        key=extract_step,
    )
    if not ckpts:
    ...
    for ckpt_path in ckpts:
        ...
        sd = load_model_state_from_ckpt(ckpt_path)
        ...
        row = {
            ...
            f"loss_pat_len_{d_patch}": l1,
            f"mae_pat_len_{d_patch}": m1,
            f"loss_pat_len_{2*d_patch}": l2,
            f"mae_pat_len_{2*d_patch}": m2,
        }
        rows.append(row)
        print(row)
