...
class Chronos2Encoder(nn.Module):
    ...
    def _expand_and_invert_time_attention_mask(
        ...
        attention_mask = attention_mask.to(dtype=floating_type)  # fp16 compatibility
    ...
    def _construct_and_invert_group_time_mask(
        ...
        # a cell is True if both row and col have the same group id
    ...
    def forward(
        ...
        *,
        ...
        # Make the time attention mask broadcastable to (batch, n_heads, q_len, kv_len) and invert
...
class Chronos2Model(PreTrainedModel):
    ...
    def __init__(self, config: Chronos2CoreConfig):
        ...
        assert self.chronos_config.input_patch_size == self.chronos_config.output_patch_size, (
            "input_patch_size and output_patch_size must be equal"
            f"but found {self.chronos_config.input_patch_size} and"
            f" {self.chronos_config.output_patch_size}"
        )
    ...
    def _validate_input(
        ...
        if context.ndim != 2:
            raise ValueError(
                f"context must have shape (batch_size, context_length), "
        ...
        if future_covariates is not None:
            if future_covariates.shape[0] != context.shape[0] or future_covariates.ndim != 2:
                raise ValueError(
                    ...
                    f"(batch_size={context.shape[0]}, future_length), "
                    f"found: {tuple(future_covariates.shape)}"
                )
            if future_covariates.shape[-1] > num_output_patches * output_patch_size:
                raise ValueError(
                    f"{num_output_patches=} must be large enough to accommodate "
                    f"the length of future_covariates, "
                    f"found: {future_covariates.shape[-1]} > {num_output_patches} * "
                    f"{output_patch_size}"
                )
            if future_target is not None and future_target.shape != future_covariates.shape:
                raise ValueError(
                    f"future_target must have the same shape as future_covariates, "
                    f"found: {tuple(future_target.shape)} and {tuple(future_covariates.shape)}"
                )
        if future_covariates_mask is not None:
            ...
            if future_covariates_mask.shape != future_covariates.shape:
                raise ValueError(
                    ...
                    f"found: {tuple(future_covariates_mask.shape)} and "
                    f"{tuple(future_covariates.shape)}"
                )
        ...
        if future_target is not None:
            if future_target.shape[0] != context.shape[0] or future_target.ndim != 2:
                raise ValueError(
                    f"future_target must have shape "
                    f"(batch_size={context.shape[0]}, future_length), "
                    f"found: {tuple(future_target.shape)}"
                )
            if future_target.shape[-1] > output_patch_size * num_output_patches:
                raise ValueError(
                    f"{num_output_patches=} must be large enough to accommodate "
                    f"the length of future_target, "
        ...
        if future_target_mask is not None:
            ...
            if future_target_mask.shape != future_target.shape:
                raise ValueError(
                    f"future_target_mask must have the same shape as future_target, "
                    f"found: {tuple(future_target_mask.shape)} and {tuple(future_target.shape)}"
                )
    ...
    def _prepare_patched_future(
        ...
        if future_covariates is not None:
            ...
            if torch.isnan(future_covariates).any():
                raise ValueError(
                    "future_covariates contains NaN values at indices not masked by "
                    "future_covariates_mask. Input the correct future_covariates_mask or omit it "
                    "to automatically infer the mask based on NaN values."
                )

            # add padding if the length of future_covariates is not an
            # integer multiple of output_patch_size
    ...
    def _compute_loss(
        ...
        median_quantile = quantile_preds.median(dim=1, keepdim=True).values
        mse_loss = (future_target - median_quantile) ** 2
        mae_loss = torch.abs(future_target - median_quantile)
        ...
        # the first components masks any missing targets and the second component
        # masks known future values
        ...
        mae_loss = mae_loss * loss_mask
        # mean over prediction horizon, mean over batch
    ...
    def encode(
        ...
        if group_ids is None:
            group_ids = torch.arange(batch_size, device=self.device, dtype=torch.long)
    ...
    def forward(
        ...
        ----------
        context : torch.Tensor
            Input context tensor of shape (batch_size, context_length) containing
            the historical values.

        context_mask : torch.Tensor | None, optional
            Binary mask tensor of shape (batch_size, context_length) indicating
            observed values in the context. If missing, the context_mask is
            inferred from NaN values in the context.

        group_ids : torch.Tensor | None, optional
            Group IDs of shape (batch_size,) indicating which time series in the
            batch form a group. A group indicates a task, for example, for a batch
            of size 6:
            - if groups_ids = [0, 1, 2, 3, 4, 5], each time series is treated
              independently.
            - if groups_ids = [0, 0, 1, 1, 1, 2], information is mixed across the
              first two time series (id=0), the next three time series (id=1) and
              the last time series is treated separately. Information is NOT shared
              among time series from different groups.
            The ordering and specific values of group_ids are not important, all
            time series with the same group ID form a group.

        future_covariates : torch.Tensor | None, optional
            Tensor of shape (batch_size, future_length) containing future
            covariates. Note that the size of tensor along the first axis is equal
            to the batch_size. This means that future values (which may be NaNs)
            must be provided for each time series in the batch. For any time series
            that need to be forecasted, the future_covariates can be set to NaNs,
            if ``future_covariates_mask`` is omitted or to an arbitrary dummy value
            when ``future_covariates_mask`` is provided. ``future_covariates`` can
            be used with ``group_ids`` to construct heterogenous forecasting tasks
            in a single batch. For example:
            - future_covariates = [[nan, ...], [nan, ...], [v1, ...], [v2, ...],
              [nan, ...], [nan, ...]]
            - groups_ids = [0, 0, 1, 1, 1, 2]
            - future_covariates_mask = None
            contains 3 types of forecasting tasks:
            - [0, 0]: The first task, both future_covariates are missing, which
              implies that the two time series need to be forecasted jointly, i.e.,
              multivariate forecasting.
            - [1, 1, 1]: In the next task, the first two future_covariates are
              available and the last one is missing ([v1, ...], [v2, ...],
              [nan, ...]), where [v1, ...] and [v1, ...] denote an arbitrary
              sequence of values. This indicates that the first two time series are
              known covariates and the third one needs to be forecasted by the
              model.
            - [2]: The last task has a single time series in the group which needs
              to be forecasted independently.
            There is no theoretical limit on the number of time series in a group,
            i.e., the number of targets and known covariates in a task. The above
            setup subsumes tasks with past-only covariates as the model's
            prediction for those time series can simply be ignored downstream.

        future_covariates_mask : torch.Tensor | None, optional
            Binary mask tensor of shape (batch_size, future_length) indicating
            observed values in the future_covariates. If missing, the
            future_covariates_mask is inferred from NaN values in the
            future_covariates.

        num_output_patches: int, optional
            Number of output patches to predict, by default 1.
            When 'future_covariates' and/or 'future_target' are provided,
            num_output_patches must be large enough to accomodate their lengths
            i.e., num_output_patches * output_patch_size >= future_length.

        future_target : torch.Tensor | None, optional
            Tensor of shape (batch_size, future_length) used during training. If
            'future_covariates' is provided, 'future_target' must have the same
            shape.

        future_target_mask : torch.Tensor | None, optional
            Binary mask tensor of shape (batch_size, future_length) indicating
            observed values in the future_target. If missing, the
            future_target_mask is inferred from NaN values in the future_target.
        ...
        Chronos2Output
            ...
            - loss (torch.Tensor | None):
                Quantile loss if 'future_target' is provided, else None.

            - quantile_preds (torch.Tensor | None):
                Predicted quantiles of shape (batch_size, num_quantiles,
                num_output_patches * output_patch_size). quantile_preds will
                contain an entry for every time series in the context batch
                regardless of whether it was a known future covariate.

            - enc_time_self_attn_weights (tuple[torch.Tensor, ...] | None):
                Time self-attention weights from the encoder if
                output_attentions = True, else None.

            - enc_group_self_attn_weights (tuple[torch.Tensor, ...] | None):
                Group self-attention weights from the encoder if
                output_attentions = True, else None.
        """
        batch_size = context.shape[0]
        (
        ...
        # Slice the last num_output_patches hidden states to be input into the
