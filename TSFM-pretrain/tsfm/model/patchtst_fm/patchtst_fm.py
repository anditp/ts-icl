...
class PatchTSTFMPreTrainedModel(PreTrainedModel):
    ...
    def all_tied_weights_keys(self):
        return getattr(self, "_all_tied_weights_keys", self.get_expanded_tied_weights_keys())
...
class PatchTSTFMModel(PatchTSTFMPreTrainedModel):
    ...
    def decode(self, x, mask, t_pad_mask=None):
        ...
        x = self.out_layer(x)
        q_raw = x.reshape(B, N, self.config.num_quantile + 1, self.config.d_patch).permute(
            0, 2, 1, 3
        )
...
class PatchTSTFMForPretraining(PatchTSTFMPreTrainedModel):
    ...
    def forward(
        ...
        if return_loss:
            ...
            quantiles = torch.tensor(self.backbone.quantile_levels, device=x_target.device).view(
                1, 1, -1
            )
            ...
            # Median quantile index for point-prediction diagnostics
            median_idx = self.config.num_quantile // 2
            x_median = q_pred[:, :, median_idx]  # (B, T) — normalised space
            x_t = x_target.squeeze(-1)  # (B, T)
            n_valid = loss_mask.sum().clamp(min=1)
            mse_loss = ((x_median - x_t) ** 2 * loss_mask).sum() / n_valid
            mae_loss = ((x_median - x_t).abs() * loss_mask).sum() / n_valid
        else:
            loss = None
            mse_loss = None
            mae_loss = None
